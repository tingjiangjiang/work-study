...
router
	...
	.post((req, res) => {
		...
		if (username == '' || password == '') {
			...
			let crypto_md5 = crypto.createHmac('md5', 'baofeng');
			crypto_md5.update(password);
			let crypto_password = crypto_md5.digest('hex');

			db.count('user', { username, password: crypto_password }, rst => {
				let { error_code, reason, data } = rst;
				if (data == 1) {
					// 登录成功后给用户写入cookie，用来做登录拦截判断
					let cookieOption = { signed: true, httpOnly: true, maxAge: 1000 * 60 * 60 };
					res.cookie('baofenglogin', username, cookieOption);
